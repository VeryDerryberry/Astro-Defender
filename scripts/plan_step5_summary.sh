...
adding1=$(grep -o 'EXPORT_LOG_EXPORT_1_ADDING=[a-z]*' "$SCRATCH/apk_evidence.txt" | tail -1 | cut -d= -f2)
...
git_head=$(git -C "$PROJECT" rev-parse --short HEAD)
